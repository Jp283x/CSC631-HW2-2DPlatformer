...
public class MapGenerator : MonoBehaviour
{
    ...
    //HurtBlock data
    public GameObject HurtBlock;
    [Range(0, 100)]
    public int hurtBlockSpawnPercentage;
            ...
            for (int y = 0; y < height; y++)
            {
                if (x > 1 && x < width - 2 && y < height - 4 && map[x, y] == 1) //check for out of bounds  and allowable tile
                {
                        ...
                        else if (psuedoRandom.Next(0, 100) < chestSpawnPercentage)
                    {
                        ...
                        else if (psuedoRandom.Next(0, 100) < hurtBlockSpawnPercentage)
                        {
                            Vector3Int p = new Vector3Int(x, y + 1, 0);
                            Instantiate(HurtBlock, p, Quaternion.identity);
                        }
